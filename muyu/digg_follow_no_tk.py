...
class DouyinHighLevelAPI:
    ...
    def __init__(self, cookie,token_token):
        ...
        print("[cookies]********************************")
        setcook = souye.headers['Set-Cookie']
        print(f"setcookies:{setcook}")
        new_cookies = {i.split("=", maxsplit=1)[0]: i.split("=", maxsplit=1)[1] for i in setcook.split("; ") if
                   len(i.split("=", maxsplit=1)) >= 2}
        print(f"newcookie:{new_cookies}")

        session.cookies.update(new_cookies)

        # for i in souye.headers:
        #     print(f"{i}------{souye.headers[i]}")
        # for i in souye.cookies:
        #     print(i)
        # print(dict(souye.cookies()))
        pattern = re.compile(r'uid%22%3A%22([0-9]{0,})%22%2C%22')   # 查找数字
